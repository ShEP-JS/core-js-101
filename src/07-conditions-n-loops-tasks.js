 ...
 *
 */
function getIntervalString(a1, b1, isStartIncluded, isEndIncluded) {
...
function isCreditCardNumber(num) {
  ...
  for (let i = 0; i < str.length; i += 2) {
    const oddNum = +str[i] * 1;
    arr.push(oddNum);
    if (str[i + 1] != null) {
      const evenNum = +str[i + 1] * 2;
      arr.push(evenNum);
    }
  }
  ...
  const result = arr.map((item) => {
    if (item >= 10) {
      return +item.toString()[1] + (+item.toString()[0]);
    }
    return item;
  });
  const sumDigits = result.reduce((sum, item) => item + sum, 0);
